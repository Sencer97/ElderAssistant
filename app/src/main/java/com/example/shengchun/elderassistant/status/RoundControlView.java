...
public class RoundControlView extends View {
    ...
    private static final String TAG = "RoundControlView";
    private int coreX;//中心点的坐标X
    private int coreY;//中心点的坐标Y

    private List<RoundMenu> roundMenus = new ArrayList<RoundMenu>();//菜单列表
    ...
    public RoundControlView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public RoundControlView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void onDraw(Canvas canvas){
        ...
        coreX = getWidth() / 2;
        coreY = getHeight() / 2;
        roundRadius = (int) (getWidth()/2 * radiusDistance);    //计算中心圆圈的半径
        RectF rect = new RectF(0, 0, getWidth(), getHeight());
        if (roundMenus != null && roundMenus.size() > 0) {
            float sweepAngle = 360 / SIZE;//每个弧形的角度
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (distanceLine <= roundRadius) {
                    ...
                    float sweepAngle = 360 / 4;//每个弧形的角度
